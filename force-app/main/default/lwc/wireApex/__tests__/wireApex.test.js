...
// A empty list of records to verify the componente does something reasonable
// when there is no data to display
const mockGetAccountListNoRecords = require('./data/getAccountListNoRecords.json');

// Register as apex wire adapter. Some tests verify the provisionoed values trigger desired behavior;
...
describe('c-wire-apex', () => {
  ...
  describe('getAccountList @wire data', () => {
    ...
    it('renders not items when no records are returned', () => {
      const element = createElement('c-wire-apex', {
        is: WireApex
      });
      document.body.appendChild(element);
